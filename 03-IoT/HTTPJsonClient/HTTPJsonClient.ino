  ...
  if (err == 0)
  {
    ...
    if (statusCode >= 0)
    {
      ...
      { 
        ...
        // Alocando espaço para o documento JSON
        // Use arduinojson.org/v6/assistant to compute the capacity.
        DynamicJsonDocument jsonObj(120);

        // Realizando o parsing dos dados JSON da String (corpo da resposta HTTP) para DynamicJsonDocument
        DeserializationError error = deserializeJson(jsonObj, body.c_str());
        if (error) {
        ...
        // Acessando chaves/valores no objeto Json de retorno
        String timestamp = jsonObj["timestamp"];
        String studentName = jsonObj["student_name"];

        Serial.println("Explorando dados no formato JSON:");
        Serial.print("HTTP Request Timestamp: ");
        Serial.println(timestamp);
        Serial.print("Random Student Name: ");
